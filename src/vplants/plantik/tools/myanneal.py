...
class base_schedule(object):
    ...
    def getstart_temp(self, best_state):
        ...
        for _ in range(self.Ninit):
            print 'INITIALISATION completed: ', 100*_/float(self.Ninit) 
...
def anneal(func, x0, args=(), schedule='fast', full_output=0,
           ...
           lower=-100, upper=100, dwell=50, Ninit=50):
    ...
    while 1:
        ...
        for n in range(dwell):
            ...
            print n, 'last cost=', last_state.cost, ' last x', last_state.x, ' dE=', dE
        ...
        if (maxaccept is not None) and (schedule.accepted > maxaccept):
            retval = 4
            break
        print '------------ Iterations=', iters, ' Temperature=',schedule.T, ' schedule.accepted=', schedule.accepted
